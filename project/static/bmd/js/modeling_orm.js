...
Session.prototype = {
    add_bmr: function(bmr){
        this.bmrs.push(bmr);
    },
    add_option: function(option, default_override){
    ...
    load_options_template: function(option){
        var opt = new OptionFile(this, option);
        opt.default_overrides();
        this.option_files.push(opt);
    },
    load_options_session: function(option){
...
_.extend(OptionFile, {
    opt_restrict_polynomial: {'0':'None', '-1':'Non-positive', '1':'Non-negative'},
    parameter_types: {'d':'Default', 's':'Specified', 'i':'Initialized'},
    variance_models: {0:'Modeled Variance', 1:'Constant Variance'},
    toggle_variance: function(model_list){
...
OptionFile.prototype = {
    ...
    build_option_field: function(id, item){
    ...
    build_option_row: function(id,item){
        //build field based on field type
        var tr = $('<tr></tr>');
    ...
    build_settings_form: function(td_details){
        //change title
    ...
    build_settings_tbl: function(td_details){
        //change title
        $('#bmd_settings_form').data('opt',this).data('td_details',td_details);
        $('#bmd_settings_form h3')
            .text('HAWC BMDS {0}: {1} Model Options'
...
BMR.prototype = {
    clone: function(){
        return new BMR(this.parent, this);
    },
    send_settings: function(){
        // send settings in json form back to server
    ...
    build_settings_form: function(row){
...
var LogicItem = function(item){
    ...
    this.threshold = parseFloat(item.threshold, 10);
...
_.extend(Tbl_Recommendation, {
    bin_names: {0: 'Viable', 1: 'Questionable', 2: 'Not Viable', 99: 'N/A'}
});
Tbl_Recommendation.prototype = {
    add_footnote_rows: function(){
    ...
    get_recommendation_field: function(model){
    ...
    build_override_form: function(outputs){
    ...
    build_override_table: function(outputs){
        var output = outputs[outputs[0].parent.output_tbl.bmr_of_interest];
        $('#override_model_description').html(output.model_name + ' (' + output.bmr.get_table_name() + ')');
    ...
    get_recommended_model: function(){
    ...
    maximum_BMDL_difference: function(){
...
_.extend(BMD_UnitTests, {
    get_numeric_value: function(v){
...
BMD_UnitTests.prototype = {
    test_cases: function(key){
    ...
    fieldExists: function(field_name){
        try{
    ...
    exists_bmd: function(){
        return this.fieldExists('BMD');
    },
    exists_bmdl: function(){
        return this.fieldExists('BMDL');
    },
    exists_bmdu: function(){
        return this.fieldExists('BMDU');
    },
    exists_aic: function(){
        return this.fieldExists('AIC');
    },
    bmd_ratio: function(){
        var ratio, BMD, BMDL;
        try{
